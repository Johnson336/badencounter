 ...
 *       - gestures: Gestures system for touch-ready devices (or simulated from
 *mouse inputs)
 *
 *
 *   LICENSE: zlib/libpng
...
static const int CursorsLUT[] = {
    ...
    // SDL_SYSTEM_CURSOR_WAIT,      // No equivalent implemented on MouseCursor
    // enum on raylib.h SDL_SYSTEM_CURSOR_WAITARROW, // No equivalent
    // implemented on MouseCursor enum on raylib.h
};
...
static KeyboardKey ConvertScancodeToKey(
    SDL_Scancode sdlScancode); // Help convert SDL scancodes to raylib key
...
void ToggleFullscreen(void) {
  ...
  if ((monitor >= 0) && (monitor < monitorCount)) {
    ...
    TRACELOG(LOG_WARNING, "SDL: Failed to find selected monitor");
}
...
void RestoreWindow(void) { SDL_ShowWindow(platform.window); }
...
void SetWindowState(unsigned int flags) {
  ...
  if (flags & FLAG_WINDOW_UNFOCUSED) {
    ...
    TRACELOG(LOG_WARNING, "SetWindowState() - FLAG_WINDOW_UNFOCUSED is not "
                          "supported on PLATFORM_DESKTOP_SDL");
  }
  if (flags & FLAG_WINDOW_TOPMOST) {
    SDL_SetWindowAlwaysOnTop(platform.window, SDL_FALSE);
  }
  if (flags & FLAG_WINDOW_ALWAYS_RUN) {
    TRACELOG(LOG_WARNING, "SetWindowState() - FLAG_WINDOW_ALWAYS_RUN is not "
                          "supported on PLATFORM_DESKTOP_SDL");
  }
  if (flags & FLAG_WINDOW_TRANSPARENT) {
    TRACELOG(LOG_WARNING, "SetWindowState() - FLAG_WINDOW_TRANSPARENT is not "
                          "supported on PLATFORM_DESKTOP_SDL");
  }
  if (flags & FLAG_WINDOW_HIGHDPI) {
    // NOTE: Such a function does not seem to exist
    TRACELOG(LOG_WARNING, "SetWindowState() - FLAG_WINDOW_HIGHDPI is not "
                          "supported on PLATFORM_DESKTOP_SDL");
  }
  if (flags & FLAG_WINDOW_MOUSE_PASSTHROUGH) {
    // SDL_SetWindowGrab(platform.window, SDL_FALSE);
    TRACELOG(LOG_WARNING, "SetWindowState() - FLAG_WINDOW_MOUSE_PASSTHROUGH is "
                          "not supported on PLATFORM_DESKTOP_SDL");
  }
...
void ClearWindowState(unsigned int flags) {
  ...
  if (flags & FLAG_WINDOW_UNFOCUSED) {
    ...
    TRACELOG(LOG_WARNING, "ClearWindowState() - FLAG_WINDOW_UNFOCUSED is not "
                          "supported on PLATFORM_DESKTOP_SDL");
  }
  if (flags & FLAG_WINDOW_TOPMOST) {
    SDL_SetWindowAlwaysOnTop(platform.window, SDL_FALSE);
  ...
  if (flags & FLAG_WINDOW_ALWAYS_RUN) {
    TRACELOG(LOG_WARNING, "ClearWindowState() - FLAG_WINDOW_ALWAYS_RUN is not "
                          "supported on PLATFORM_DESKTOP_SDL");
  }
  if (flags & FLAG_WINDOW_TRANSPARENT) {
    TRACELOG(LOG_WARNING, "ClearWindowState() - FLAG_WINDOW_TRANSPARENT is not "
                          "supported on PLATFORM_DESKTOP_SDL");
  }
  if (flags & FLAG_WINDOW_HIGHDPI) {
    // NOTE: There also doesn't seem to be a feature to disable high DPI once
    // enabled
    TRACELOG(LOG_WARNING, "ClearWindowState() - FLAG_WINDOW_HIGHDPI is not "
                          "supported on PLATFORM_DESKTOP_SDL");
  }
  if (flags & FLAG_WINDOW_MOUSE_PASSTHROUGH) {
    // SDL_SetWindowGrab(platform.window, SDL_TRUE);
    TRACELOG(LOG_WARNING, "ClearWindowState() - FLAG_WINDOW_MOUSE_PASSTHROUGH "
                          "is not supported on PLATFORM_DESKTOP_SDL");
  }
  if (flags & FLAG_BORDERLESS_WINDOWED_MODE) {
    SDL_SetWindowFullscreen(platform.window, 0);
  }
  if (flags & FLAG_MSAA_4X_HINT) {
    SDL_GL_SetAttribute(SDL_GL_MULTISAMPLEBUFFERS,
                        0); // Disable multisampling buffers
    SDL_GL_SetAttribute(SDL_GL_MULTISAMPLESAMPLES, 0); // Disable multisampling
  }
  if (flags & FLAG_INTERLACED_HINT) {
    TRACELOG(LOG_WARNING, "ClearWindowState() - FLAG_INTERLACED_HINT is not "
                          "supported on PLATFORM_DESKTOP_SDL");
  }
}
...
void SetWindowIcon(Image image) {
  ...
  iconSurface =
      SDL_CreateRGBSurfaceFrom(image.data, image.width, image.height, depth,
...
void SetWindowMonitor(int monitor) {
  ...
  if ((monitor >= 0) && (monitor < monitorCount)) {
    // NOTE:
    // 1. SDL started supporting moving exclusive fullscreen windows between
    // displays on SDL3,
    //    see commit
    //    https://github.com/libsdl-org/SDL/commit/3f5ef7dd422057edbcf3e736107e34be4b75d9ba
    // 2. A workround for SDL2 is leaving fullscreen, moving the window, then
    // entering full screen again.
    const bool wasFullscreen =
        ((CORE.Window.flags & FLAG_FULLSCREEN_MODE) > 0) ? true : false;
    ...
    if (SDL_GetDisplayUsableBounds(monitor, &usableBounds) == 0) {
      if (wasFullscreen == 1)
        ToggleFullscreen(); // Leave fullscreen.

      // If the screen size is larger than the monitor usable area, anchor it on
      // the top left corner, otherwise, center it
      if ((screenWidth >= usableBounds.w) || (screenHeight >= usableBounds.h)) {
        // NOTE:
        // 1. There's a known issue where if the window larger than the target
        // display bounds,
        //    when moving the windows to that display, the window could be
        //    clipped back ending up positioned partly outside the target
        //    display.
        ...
        // 3. It was't done here because we can't assume changing the window
        // size automatically
        ...
        const int y =
            usableBounds.y + (usableBounds.h / 2) - (screenHeight / 2);
        SDL_SetWindowPosition(platform.window, x, y);
        CORE.Window.position.x = x;
        CORE.Window.position.y = y;
      }
      ...
      TRACELOG(LOG_WARNING,
               "SDL: Failed to get selected display usable bounds");
  } else
    TRACELOG(LOG_WARNING, "SDL: Failed to find selected monitor");
}
...
// Set window focused
void SetWindowFocused(void) { SDL_RaiseWindow(platform.window); }

// Get native window handle
void *GetWindowHandle(void) { return (void *)platform.window; }
...
void SetClipboardText(const char *text) { SDL_SetClipboardText(text); }

// Get clipboard text content
// NOTE: returned string must be freed with SDL_free()
const char *GetClipboardText(void) { return SDL_GetClipboardText(); }
...
void SwapScreenBuffer(void) { SDL_GL_SwapWindow(platform.window); }
...
double GetTime(void) {
  unsigned int ms =
      SDL_GetTicks(); // Elapsed time in milliseconds since SDL_Init()
...
void OpenURL(const char *url) {
  ...
  if (strchr(url, '\'') != NULL)
    TRACELOG(LOG_WARNING, "SYSTEM: Provided URL could be potentially "
                          "malicious, avoid [\'] character");
  else
    SDL_OpenURL(url);
}
...
void PollInputEvents(void) {
  ...
  // TODO: It resets on target platform the mouse position and not filled again
  // until a move-event, so, if mouse is not moved it returns a (0, 0)
  // position... this behaviour should be reviewed!
  // for (int i = 0; i < MAX_TOUCH_POINTS; i++) CORE.Input.Touch.position[i] =
  // (Vector2){ 0, 0 };

  // Map touch position to mouse position for convenience
  // WARNING: If the target desktop device supports touch screen, this behavious
  // should be reviewed!
  ...
  int touchAction =
      -1; // 0-TOUCH_ACTION_UP, 1-TOUCH_ACTION_DOWN, 2-TOUCH_ACTION_MOVE
  ...
  for (int i = 0; i < MAX_MOUSE_BUTTONS; i++)
    CORE.Input.Mouse.previousButtonState[i] =
        CORE.Input.Mouse.currentButtonState[i];

  // Poll input events for current plaform
  //-----------------------------------------------------------------------------
  /*
  // WARNING: Indexes into this array are obtained by using SDL_Scancode values,
  not SDL_Keycode values const Uint8 *keys = SDL_GetKeyboardState(NULL); for
  (int i = 0; i < 256; ++i)
  {
  ...
  while (SDL_PollEvent(&event) != 0) {
    ...
    case SDL_DROPFILE: // Dropped file
    {
      if (CORE.Window.dropFileCount == 0) {
        // When a new file is dropped, we reserve a fixed number of slots for
        // all possible dropped files at the moment we limit the number of drops
        // at once to 1024 files but this behaviour should probably be reviewed
        // TODO: Pointers should probably be reallocated for any new file
        // added...
        CORE.Window.dropFilepaths = (char **)RL_CALLOC(1024, sizeof(char *));

        CORE.Window.dropFilepaths[CORE.Window.dropFileCount] =
            (char *)RL_CALLOC(MAX_FILEPATH_LENGTH, sizeof(char));
        strcpy(CORE.Window.dropFilepaths[CORE.Window.dropFileCount],
               event.drop.file);
        ...
        TRACELOG(LOG_WARNING, "FILE: Maximum drag and drop files at once is "
    ...
    case SDL_MOUSEBUTTONDOWN: {
      // NOTE: SDL2 mouse button order is LEFT, MIDDLE, RIGHT, but raylib uses
      // LEFT, RIGHT, MIDDLE like GLFW
      //       The following conditions align SDL with raylib.h MouseButton enum
      //       order
    ...
    if (gestureUpdate) {
      ...
      // Normalize gestureEvent.position[0] for CORE.Window.screen.width and
...
int InitPlatform(void) {
  ...
  // if ((CORE.Window.flags & FLAG_WINDOW_HIDDEN) == 0) flags |=
  // SDL_WINDOW_HIDDEN;
  ...
  // if ((CORE.Window.flags & FLAG_WINDOW_TRANSPARENT) > 0) flags |=
  // SDL_WINDOW_TRANSPARENT;     // Alternative: SDL_GL_ALPHA_SIZE = 8

  // if ((CORE.Window.flags & FLAG_FULLSCREEN_DESKTOP) > 0) flags |=
  ...
  if (rlGetVersion() == RL_OPENGL_21) {
    ...
    SDL_GL_SetAttribute(
        ...
        SDL_GL_CONTEXT_FORWARD_COMPATIBLE_FLAG); // OSX Requires forward
                                                 // compatibility
#endif
    ...
    SDL_GL_SetAttribute(
        SDL_GL_CONTEXT_FLAGS,
        SDL_GL_CONTEXT_DEBUG_FLAG); // Enable OpenGL Debug Context
#endif
  ...
  platform.window = SDL_CreateWindow(
      CORE.Window.title, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED,
      CORE.Window.screen.width, CORE.Window.screen.height, flags);
  ...
  if ((platform.window != NULL) && (platform.glContext != NULL)) {
    ...
    TRACELOG(LOG_INFO, "    > Display size: %i x %i", CORE.Window.display.width,
             CORE.Window.display.height);
    TRACELOG(LOG_INFO, "    > Screen size:  %i x %i", CORE.Window.screen.width,
             CORE.Window.screen.height);
    TRACELOG(LOG_INFO, "    > Render size:  %i x %i", CORE.Window.render.width,
             CORE.Window.render.height);
    TRACELOG(LOG_INFO, "    > Viewport offsets: %i, %i",
  ...
  if (SDL_NumJoysticks() >= 1) {
    ...
    // if (platform.gamepadgamepad == NULL) TRACELOG(LOG_WARNING, "PLATFORM:
    // Unable to open game controller [ERROR: %s]", SDL_GetError());
  }
